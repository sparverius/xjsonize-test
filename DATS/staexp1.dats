...
//
| G1Elist(g1es) =>
  jsonify("G1Elist", "g1es", jsonize_list<g1exp>("g1explst", g1es))
//
| G1Enone(loc) =>
  jsonify("G1Enone")
//
...
x0.node() of
| G1MARGsarg(g1as) =>
  jsonify("G1MARGsarg", "g1as", jsonize_list<g1arg>("g1arglst", g1as))
| G1MARGdarg(g1as) =>
  jsonify("G1MARGdarg", "g1as", jsonize_list<g1arg>("g1arglst", g1as))
...
//
| S1Tlist(s1ts) =>
  jsonify("S1Tlist", "s1ts", jsonize_list<sort1>("sort1lst", s1ts))
//
| S1Tqual(tok0, s1t1) =>
...
| D1TSORT(tid, s1cs) =>
  jsonify("D1TSORT", ("tid", "s1cs"),
    (jsonize(tid), jsonize_list<s1rtcon>("s1rtconlst", s1cs))
  )
...
| S1RTDEFsbst(s1a0, s1es) =>
  jsonify("S1RTDEFsbst(", ("s1a0", "s1es"),
    (jsonize(s1a0), jsonize_list<s1exp>("s1explst", s1es))
  )
...
| S1ARGsome(tok, opt) =>
  jsonify("S1ARGsome(", ("tok", "opt"),
    (jsonize(tok), jsonize_option<sort1>("sort1opt", opt))
  )
) (* jsonize_s1arg *)
end
...
*)
| S1MARGlist(s1as) =>
  jsonify("S1MARGlist", "s1as", jsonize_list<s1arg>("s1arglst", s1as))
) (* jsonize_s1marg *)
end
...
| T1ARGsome(tok, opt) =>
  jsonify("T1ARGsome", ("tok", "opt"),
    (jsonize(tok), jsonize_option<token>("tokenopt", opt))
  )
) (* jsonize_t1arg *)
end
...
*)
| T1MARGlist(t1as) =>
  jsonify("T1MARGlist", "t1as", jsonize_list<t1arg>("t1arglst", t1as))
) (* jsonize_t1marg *)
end
...
| S1QUAvars(ids, opt) =>
  jsonify("S1QUAvars(", ("ids", "opt"),
    (
      jsonize_list<token>("tokenlst", ids),
      jsonize_option<sort1>("sort1opt", opt)
    )
  )
)
...
case+ x0.node() of
| S1UNIsome(s1qs) =>
  jsonify("S1UNIsome", "s1qs", jsonize_list<s1qua>("s1qualst", s1qs))
)
end
...
//
| S1Eimp(s1es) =>
  jsonify("S1Eimp", "s1es", jsonize_list<s1exp>("s1explst", s1es))
//
(*
| S1Eapp(s1e0, s1es) =>
...
//
| S1Elist(s1es) =>
  jsonify("S1Elist", "s1es", jsonize_list<s1exp>("s1explst", s1es))
| S1Elist(s1es1, s1es2) =>
  jsonify("S1Elist", ("s1es1", "s1es2"),
    (
      jsonize_list<s1exp>("s1explst", s1es1),
      jsonize_list<s1exp>("s1explst", s1es2)
    )
  )
//
| S1Etuple(k0, s1es) =>
  jsonify("S1Etuple", ("k0", "s1es"),
    (jsonize(k0), jsonize_list<s1exp>("s1explst", s1es))
  )
...
| S1Erecord(k0, ls1es) =>
  jsonify("S1Erecord", ("k0", "ls1es"),
    (
      jsonize(k0),
      jsonize_list<sl0abled(s1exp)>("labs1explst", ls1es)
    )
  )
| S1Erecord(k0, ls1es1, ls1es2) =>
  jsonify("S1Erecord", ("k0", "ls1es1", "ls1es2"),
    (
      jsonize(k0),
      jsonize_list<sl0abled(s1exp)>("labs1explst", ls1es1),
      jsonize_list<sl0abled(s1exp)>("labs1explst", ls1es2)
    )
  )
//
| S1Eforall(s1qs) =>
  jsonify("S1Eforall", "s1qs", jsonize_list<s1qua>("s1qualst", s1qs))
| S1Eexists(k0, s1qs) =>
  jsonify("S1Eexists", ("k0", "s1qs"),
    (jsonize(k0), jsonize_list<s1qua>("s1qualst", s1qs))
  )
//
| S1Elam(arg, res, s1e) =>
...
| D1ATYPE(tok, arg, res, d1cs) =>
  jsonify("D1ATYPE", ("tok", "arg", "res", "d1cs"),
    (
      ...
      jsonize_option<sort1>("sort1opt", res),
      jsonize_list<d1atcon>("d1atconlst", d1cs)
    )
  )
...
| D1ATCON(s1us, tok, s1is, argopt) =>
  jsonify("D1ATCON", ("s1us", "tok", "s1is", "argopt"),
    (
      jsonize_list<s1uni>("s1unilst", s1us),
      jsonize(tok),
      jsonize_list<s1exp>("s1explst", s1is),
      jsonize_option<s1exp>("s1expopt", argopt)
    )
  )
