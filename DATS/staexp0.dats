...
//
| G0Eapps(s0ts) =>
  jsonify("GOEapps", "s0ts", jsonize_list<g0exp>("g0explst", s0ts))
//
| G0Elist(t0, g0es, t1) =>
  jsonify("GOElist", ("t0", "g0es", "t1"),
    (jsonize(t0), jsonize_list<g0exp>("g0explst", g0es), jsonize(t1))
  )
//
| G0Enone(tok) =>
...
| G0MARGsarg(tbeg, g0as, tend) =>
  jsonify("G0MARGsarg", ("tbeg", "g0as", "tend"), rst) where
    val rst = (jsonize(tbeg), jsonize_list<g0eid>("g0eidlst", g0as), jsonize(tend))
  end
...
//
| S0Tapps(s0ts) =>
  jsonify("S0Tapps", "s0ts", jsonize_list<sort0>("sort0lst", s0ts))
//
| S0Tlist(t0, s0ts, t1) =>
  jsonify("S0Tlist", ("t0", "s0ts", "t1"), rst) where
    val js0ts = jsonize_list<sort0>("sort0lst", s0ts)
...
| D0TSORT(tid, tok, s0cs) =>
  jsonify("D0RTCON", ("tid", "tok", "s0cs"), (
      jsonize(tid),
      jsonize(tok),
      jsonize_list<s0rtcon>("s0rtconlst", s0cs)
    )
  )
)
...
| T0ARGsome(tid, opt) =>
  jsonify("T0ARGsome", ("sid", "opt"),
    (jsonize(tid), jsonize_option<token>("tokenopt", opt))
  )
)
...
| T0MARGlist(tbeg, t0as, tend) =>
  jsonify("T0MARGlist", ("tbeg", "t0as", "tend"),
    (
      jsonize(tbeg),
      jsonize_list<t0arg>("t0arglst", t0as),
      jsonize(tend)
    )
  )
)
...
| S0QUAvars(ids, opt) =>
  jsonify("S0QUAprop", ("ids", "opt"),
    (jsonize_list<i0dnt>("i0dntlst", ids), jsonize_option<sort0>("sort0opt", opt))
  )
)
end
...
//
| S0Eapps(s0es) =>
  jsonify("S0Eapps", "s0es", jsonize_list<s0exp>("s0explst", s0es))
//
| S0Eimp(tbeg, s0es, tend) =>
  jsonify("S0Eimp", ("tbeg", "s0es", "tend"),
    (
      jsonize(tbeg),
      jsonize_list<s0exp>("s0explst", s0es),
      jsonize(tend)
...
| S0Eparen
  (tbeg, s0es, tend) =>
  jsonify("S0Eparen", ("tbeg", "s0es", "tend"),
    (
      jsonize(tbeg),
      jsonize_list<s0exp>("s0explst", s0es),
      jsonize(tend)
...
| S0Eforall(tbeg, s0qs, tend) =>
  jsonify("S0Eforall", ("tbeg", "s0qs", "tend"),
    (
      jsonize(tbeg),
      jsonize_list<s0qua>("s0qualst", s0qs),
      jsonize(tend)
...
| S0Eexists(tbeg, s0qs, tend) =>
  jsonify("S0Eexists", ("tbeg", "s0qs", "tend"),
    (
      jsonize(tbeg),
      jsonize_list<s0qua>("s0qualst", s0qs),
      jsonize(tend)
...
| S0Etuple(tbeg, topt, s0es, tend) =>
  jsonify("S0Etuple", ("tbeg", "topt", "s0es", "tend"),
    (
      jsonize(tbeg),
...
| S0Erecord(tbeg, topt, s0es, tend) =>
  jsonify("S0Erecord", ("tbeg", "topt", "s0es", "tend"),
    (
      jsonize(tbeg),
      jsonize_option<token>("tokenopt", topt),
      jsonize_list<sl0abled(s0exp)>("labs0explst", s0es),
      jsonize(tend)
    )
  )
//
| S0Elam(tbeg, arg0, res1, tok1, s0e0, tend) =>
  jsonify("S0Elam", ("tbeg", "arg0", "res1", "tok1", "s0e0", "tend"),
    (
      jsonize(tbeg),
      jsonize_list<s0marg>("s0marglst", arg0),
      jsonize_option<sort0>("sort0opt", res1),
      jsonize(tok1),
      jsonize(s0e0),
...
//
| S0Equal(tok, s0e) =>
  jsonify("S0Equal", ("tok", "s0e"), (jsonize(tok), jsonize(s0e)))
//
| S0Enone(tok) =>
...
| s0exp_RPAREN_cons1(tok1, s0es, tok2) =>
  jsonify("s0exp_RPAREN_cons1", ("tok1", "s0es", "tok2"),
    (
      jsonize(tok1),
      jsonize_list<s0exp>("s0explst", s0es),
      jsonize(tok2)
...
| labs0exp_RBRACE_cons1(tok1, ls0es, tok2) =>
  jsonify("labs0exp_RBRACE_cons0", ("tok1", "ls0es", "tok2"),
    (
      jsonize(tok1),
      jsonize_list<labs0exp>("labs0explst", ls0es),
      jsonize(tok2)
    )
  )
)
...
| D0ATYPE(tid, arg, res, teq, d0cs) =>
  jsonify("D0ATYPE", ("tid", "arg", "res", "teq", "d0cs"),
    (
      jsonize(tid),
      jsonize_list<t0marg>("t0marglst", arg),
      jsonize_option<sort0>("sort0opt", res),
      jsonize(teq),
      jsonize_list<d0atcon>("d0atconlst", d0cs)
    )
  )
)
...
| D0ATCON(s0us, dcon, s0is, argopt) =>
  jsonify("D0ATCON", ("s0us", "dcon", "s0is", "argopt"),
    (
      jsonize_list<s0uni>("s0unilst", s0us),
      jsonize(dcon),
      jsonize_list<s0exp>("s0explst", s0is),
      jsonize_option<s0exp>("s0expopt", argopt)
    )
  )
  where
    val jargopt = (
